...
$(() => {
  ...
  vc.events.on('shortcodes:vc_row shortcodes:vc_column shortcodes:nk_awb', (e) => {
    ...
    $icon.html(
      `<span class="vc_control-awb-overlay" style="background-color: ${
        params.awb_color || 'transparent'
      };"></span>`
    );
    ...
    if (params.awb_image) {
      ...
      switch (e.getParam('source')) {
        ...
        default:
          ...
          _.isEmpty(params.awb_image) && e.getParam('source') !== 'featured_image'
            ? _.isUndefined(imageSrc) ||
              ($model.removeData('field-awb_image-attach-image'),
