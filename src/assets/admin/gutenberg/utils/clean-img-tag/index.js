...
export default function cleanImgTag(str) {
  ...
  const attributeRegex = /(\w+)\s*=\s*["']?((?:.(?!["']?\s+(?:\S+)=|\s*\/?[>"']))+.)["']?/gi;

  result = result.replace(imgRegex, (match, attributes) => {
    const safeAttributes = attributes.replace(attributeRegex, (attr, name, value) => {
      ...
      if (lowercaseName === 'src' && /^(javascript|data):/i.test(value)) {
        return '';
      }
